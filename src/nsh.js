...
const { fetchOTPFromMailbox } = require("./lib/imap");
...
const NUSUK_FROM_EMAIL = "noreply@notifications.nusuk.sa";
const getLocalIP = () => {
...
console.log(
  isEnglishOnly()
    ? `🌐 [NETWORK] Machine IP: ${getLocalIP()}`
    : `🌐 [NETWORK] Machine IP: ${getLocalIP()} | عنوان الجهاز متاح`,
);
...
async function getOTPCode() {
  ...
  try {
    if (pageMode.includes("Registration") || pageMode.includes("التسجيل")) {
      const code = await fetchOTPFromMailbox(
        getOTPEmailAddress((passenger.email || emailAddress || suggestEmail(util.getSelectedTraveler("nsh"))).split("/")[0]),
        global.adminEmailPassword,
        data.system.username.replace("@", ""),
        {
          fromEmail: NUSUK_FROM_EMAIL,
          subjects: ["Email Activation", "تفعيل البريد الالكتروني"],
          includeForwardSubjects: true,
          includeAdminMailboxRecipient: true,
          onlyUnseen: true,
          markSeen: true,
          mostRecentOnly: true,
          notBeforeDate: new Date(Date.now() - 60 * 1000),
          debug: verboseDebug,
        }
      );
      if (code) {
        await pasteOTPCode(code);
      }
    } else if (
      ...
      const code = await fetchOTPFromMailbox(
        ...
        {
          fromEmail: NUSUK_FROM_EMAIL,
          subjects: ["One Time Password", "رمز سري لمرة واحدة"],
          includeForwardSubjects: true,
          includeAdminMailboxRecipient: true,
          onlyUnseen: true,
          markSeen: true,
...
async function getCompanionOTPCode() {
  ...
  try {
    ...
    if (code) {
      await pasteOTPCodeCompanion(code);
    }
  } catch (e) {
